...
| **MM fase 5 · Dashboard executivo + score de fornecedor + sugestão de compra** — fecha o módulo MM. Lib pura `mm_dashboard.js`: `consolidarMM` (gaps do pipeline: sem engenharia / sem cotação / sem PPAP / críticos), `scoreFornecedor` (0–100 ponderando prazo=OTIF, qualidade=PPAP+avaliação, comercial=preço/atendimento, renormalizado sobre as dimensões com dado), `classificarFornecedor` (A/B/C/D), `sugestaoCompra` (ação por item: liberar engenharia / definir fornecedor / gerar RFQ). `GET /api/mm/dashboard?veiculos=N` (gaps + resumo MRP + sugestão; gate perfil fornecedor 403), `GET /api/mm/fornecedores/score` (OTIF+PPAP+avaliações). Painel executivo + ranking de score na página MM | ✅ lib pura + Express + front + testes |
| **MM varredura adversarial · 6 bugs fechados** — **BUG grave no MRP**: o mesmo part_number em 2 nós da BOM (parafuso em 2 sistemas) tinha o estoque contado **em dobro** → faltante escondido e "veículos possíveis" otimista (risco real de parada de linha); agora o MRP **agrega por PN** antes de comparar com o saldo (BUY vence MAKE, criticidade = a maior). **BUG na explosão**: pai com qtd_veiculo 0 (conjunto opcional) não zerava a subárvore (herdava 1) → necessidade inflada; e qtd 0 era coagida para 1 no POST/PUT. **Brechas de tenant**: PPAP e amostra aceitavam `fornecedor_id` de outro tenant (400 agora). **Entradas inválidas**: amostra com quantidade ≤ 0 e material com qtd_veiculo negativa (400 agora). Suíte de regressão `mm_varredura.test.js` | ✅ 6 fixes + regressões |
| **MM fase 7 · MRP → RC automática dos faltantes** — fecha o loop *detectar gap → agir*: `POST /api/mm/mrp/gerar-rc` transforma a lista de faltantes do MRP (BUY, agregados por PN) em **requisição de compra** no ciclo existente (RC→aprovação→pedido), com quantidade = faltante, custo estimado do custo médio do estoque, WBS `MRP`, prioridade Alta e observação com o gargalo ("estoque cobre X de N veículos"). Seleção opcional de materiais (`material_ids`); 400 se nada falta (não cria RC vazia). Botão "Gerar RC dos faltantes" no painel MRP | ✅ Express + front + testes |
| **MM fase 8 · Bloco industrial no Painel Executivo do CEO** — o cockpit (H1) ganha o eixo industrial: disponibilidade MRP, veículos possíveis/alvo, itens sem engenharia (bloqueiam sourcing), sem PPAP (bloqueiam produção) e críticos, condicionado ao tenant usar o MM (`industrial: null` senão). **Riscos priorizados**: estoque não cobre o plano (alto/Produção), sem PPAP (médio/Qualidade), sem engenharia (médio/Engenharia). `?veiculos=N` parametriza o plano. Reusa `consolidarMM`/`calcularMRP` | ✅ Express + front + testes |
| **PP fase 1 · Ordens de produção com gate e baixa da BOM** — o elo final do ciclo industrial: tabela `pp_ordens` (Planejada → Liberada → Em Produção → Concluída) isolada por tenant. **Gate de liberação** reusa os dois pilares do MM: bloqueia se há item BUY sem PPAP (`bloqueiosProducao`) OU se o estoque não cobre a ordem (`calcularMRP` para a qtd da OP). **Apontar produção** baixa os componentes BUY do estoque pela BOM agregada (tudo-ou-nada, com trilha em `almoxarifado_movimentos` tipo Saída/documento OP) e conclui a ordem ao atingir o plano. Lib pura `pp_producao.js` (gateLiberacaoOP, consumoDaOrdem, validarConsumo, statusAposApontamento). `POST/GET /api/pp/ordens`, `GET :id/gate` (dry-run), `POST :id/liberar`, `POST :id/apontar`. Página "PP — Ordens de Produção" (KPIs plan×real, barra de progresso, liberar/apontar) | ✅ lib pura + Express + front + testes |
| **PP varredura adversarial · 2 gaps fechados** — **GAP 1 (recall de qualidade)**: `apontar` não re-checava o PPAP — um PPAP **rejeitado depois da liberação** da ordem não parava a produção (a peça reprovada continuaria sendo montada); agora o apontamento re-checa `bloqueiosProducao` e suspende com 409 "recall" até um PPAP que libere voltar a vigorar. **GAP 2**: status "Cancelada" existia no front mas não havia endpoint — `POST /api/pp/ordens/:id/cancelar` (qualquer status exceto Concluída/Cancelada; material consumido permanece; motivo no log) + botão no front. Extras validados: consumo fracionário (0.5 kg/veíc) baixa saldo correto; apontar em cancelada 409; cancelar 2× 409. Regressões em `pp_varredura.test.js` | ✅ 2 fixes + regressões |
| **PP fase 2 · Calendário de produção + custo da OP na margem** — fecha o elo **produção ↔ financeiro**. Tabelas `pp_apontamentos` (histórico: veículos, data, **custo dos materiais** ao custo médio vigente) e `pp_plano` (plano mensal, upsert por mês). Lib pura `pp_calendario.js`: `custoConsumo` e `montarCalendario` (plan × real por mês, %, acumulados, status Concluído/Em andamento/**Atrasado**/Planejado — a aba "Calendário de Produção" da planilha). `POST/GET /api/pp/plano`, `GET /api/pp/calendario?ano`. **OP com `contrato_id`** (validado no tenant): o custo de produção entra na **margem do contrato** como linha própria "(-) Custo de produção (OPs)" em `_margemDoContrato` — material comprado-para-estoque que virou veículo agora conta no resultado. Calendário + plano mensal na página PP | ✅ lib pura + Express + front + testes |
| **PP fase 2 varredura · 3 bugs fechados** — (1) **custo cross-ano**: o resumo do calendário somava o custo de TODOS os apontamentos, de qualquer ano — calendário de 2027 mostrava custo de 2026; agora real E custo são só do ano exibido. (2) **data de apontamento sem validação**: `data: "garbage"` era persistida e o real sumia silenciosamente do calendário — agora 400 para formato não-ISO (retroativa válida continua legítima). (3) **mês 13**: o regex do plano aceitava `2027-13` (invisível no calendário, mas persistido) — agora 01–12. Regressões em `pp_calendario.test.js` | ✅ 3 fixes + regressões |
| **WMS fase 1 · Endereçamento de estoque + separação (picking)** — o almoxarifado sabe QUANTO existe; o WMS sabe **ONDE**. Tabelas `wms_enderecos` (bins com zona/capacidade) e `wms_alocacoes` (item × endereço × qtd), isoladas por tenant. Lib pura `wms.js`: `saldoEnderecado`/`saldoNaoEnderecado` (invariante: soma das alocações ≤ saldo do almoxarifado; sobra = "a guardar"), `validarAlocacao`, `sugerirPicking` (esvazia o maior bin primeiro, reporta faltante), `ocupacaoEndereco`. `POST/GET /api/wms/enderecos` (código único, ocupação %), `GET /api/wms/enderecos/:id/itens`, `POST /api/almoxarifado/:id/alocar` (409 se exceder o não endereçado), `.../mover` (transferência entre bins), `.../enderecos` (posição), `.../picking?quantidade` (sugestão de separação). Página "WMS — Endereçamento & Separação" | ✅ lib pura + Express + front + testes |
| **Auditoria do menu + Exportação Excel (Processos)** — auditoria integral menu↔rotas↔páginas: **5 itens SaaS mortos** (renders existiam em `saas_admin.js`, rotas nunca registradas → "Módulo em desenvolvimento"), **kpi_exec duplicado** no menu, **label enganoso** ("Aprovação de OS" abria o fluxo de aprovação de RC), 12 PAGE_META faltantes, 12 ícones ambíguos desduplicados, LGPD×2 desambiguado (tenant × SaaS), tooltips automáticos (title=label). **BUG pré-existente**: `GET /api/rc?q=` dava 500 (`rc.descricao` não existe) → corrigido p/ observacoes. **Exportação para Excel** no módulo Processos (Requisições): lib pura `xlsx.js` gera **.xlsx REAL** (ZIP/OOXML sem dependências: cabeçalho negrito, 1ª linha congelada, autofiltro, larguras, formatos nativos de data/moeda/número, texto como inline string → **fórmulas nunca executam**); `GET /api/rc/export.xlsx` respeita busca/status múltiplo/período, tenant e perfil (fornecedor 403), 404 claro sem registros, nome `processos_YYYY-MM-DD_HHMM.xlsx`, auditado no log; botão "Exportar para Excel" com loading/anti-duplo-clique (substitui CSV que ignorava filtros e tinha formula injection). **Teste de integridade do menu** (`menu_integridade.test.js`) vira regressão permanente | ✅ auditoria + fixes + lib pura + Express + front + testes |
| **Página Requisições migrada para a API** — fecha a pendência nº 1 da auditoria: a listagem lia do localStorage (demo por máquina) enquanto a exportação já usava a API. Agora `/api/rc` é a **fonte de verdade** (multi-tenant, por perfil): `_reqCarregarRCsAPI` + `_reqAdaptarRC` (servidor→formato da tela) alimentam KPIs, tabela, filtros, busca e detalhe sem reescrever a página; RCs locais legadas seguem **visíveis via merge** (nada some) e `_reqSaveRC` **nunca persiste linhas da API** (anti-duplicação); **aprovar** em linha da API chama `POST /api/rc/:id/aprovar` (auditado); status do servidor (Rascunho/Pendente/Aprovada/Atendida/Cancelada) ganharam badge/KPI/filtros; sem servidor a página degrada para o modo local (demo offline) | ✅ front + testes |
| **Emissão de RC via API (fase 2 da migração)** — os DOIS modais de criação (Nova Requisição em `suprimentos.js` e RC Avulsa em `fluxo_aprovacao_rc.js`) agora criam a RC no **servidor** via helper compartilhado `_reqCriarRCViaAPI` (`POST /api/rc`): payload mapeado (qtd/valor_unit → quantidade/valor_unitario_estimado), tipo em qualquer caixa (o servidor normaliza), WBS da linha do item. **Validação do servidor bloqueia** (toast + não salva local algo inválido); **sem servidor** cai no fluxo local/demo. RC criada aparece na lista da API na hora — o ciclo criar→aprovar→exportar é 100% servidor | ✅ front + testes |
| **Exportação .xlsx estendida (Pedidos, Contas a Pagar, OS, Fornecedores)** — o padrão provado no módulo Processos vira infraestrutura: helpers `_enviarXLSX` (404 claro, nome `<base>_data_hora.xlsx`, auditoria no log) e `_filtrosExportXLSX` (status múltiplo `a|b` + período `de/ate`) no servidor; o export de RC foi refatorado para usá-los. 4 endpoints novos espelhando os filtros das listagens (`/api/pedidos|contas-pagar|os|fornecedores/export.xlsx`; contas a pagar filtra período pelo **vencimento**), todos com gate de perfil fornecedor 403. Front: helper global `nexusBaixarXLSX` (loading/anti-duplo-clique/toasts) em `nexus_api.js`; os 3 CSVs legados (contas, pedidos, fornecedores) — **todos com formula injection** — retargetados para .xlsx; botão novo na página OS | ✅ helpers + 4 endpoints + front + testes |
| **Reestruturação profissional do menu (11 seções) + seções recolhíveis** — as 9 seções antigas (com "Análises" virando depósito de 16 itens díspares) reorganizadas em **11 categorias claras** seguindo o fluxo do usuário: Visão Geral · Comercial · Operações · Suprimentos & Compras · Industrial & Estoque · Fornecedores · Financeiro & Fiscal · SSMA & Compliance · Relatórios & Análises · Administração · SaaS/Negócio. Reagrupamento **verbatim** (cada `<a>` preservado com onclick/ícone/badge; asserção de integridade: 69 itens, zero perda/duplicata). **Seções recolhíveis** por clique no rótulo (chevron animado, estado persistido por usuário em localStorage, acessível por teclado Enter/Espaço com aria-expanded) — progressivo (sem JS, tudo aberto). Teste `menu_integridade.test.js` trava a ordem das 11 seções e a contagem | ✅ front + testes |
| **Diagnóstico "módulos quebrados" + seed de demonstração abrangente** — auditoria objetiva provou que **nenhum módulo estava quebrado** (0 erros 500; front de fornecedores/WBS com loading+erro+retry corretos): as telas abriam **vazias** porque o `/api/demo/seed` só populava 6 módulos do roteiro comercial e os módulos industriais (MM/PP/WMS/SSMA + funil de compras + financeiro) nunca recebiam dados. `_demoSeedModulos` agora semeia **um cenário industrial coerente em TODOS os módulos**: BOM multinível do VBTP Guarani II (com peça BUY sem liberação exercitando o gate de engenharia), amostra+PPAP aprovado, OP com apontamento e plano mensal, endereços/alocações WMS, almoxarifado, ocorrência SSMA+CAT+EPIs (1 vencido)+treinamentos (1 vencido), RC→RFQ→cotações→mapa, recebimento, NF, contas a receber, proposta, projetos, colaboradores e apontamentos de hora. **Auto-idempotente** (marcador = raiz da BOM) e com **top-up**: tenants que já rodaram o seed antigo recebem os módulos faltantes ao re-semear (números SSMA/CAT gerados por contagem global — a raiz do bug de colisão UNIQUE). Modal do botão "Cenário demo" informa quando os módulos foram completados | ✅ Express + front + testes |
| **Telas RFQ e Mapa Comparativo migradas para a API** — continua a aposentadoria do localStorage no funil de compras (após Requisições). `procurement.js` (o motor de fluxo, 5800+ linhas) passa a ler `/api/rfq` e `/api/mapas` como **fonte de verdade multi-tenant**: adapters servidor→tela (`_rfqAdaptarAPI`/`_mapaAdaptarAPI`, traduzem status Aberta/Em Análise/Aprovada→vocabulário da tela e marcam `origem:'api'`) + loaders com cache (`_procCarregarRFQAPI`/`_procCarregarMapasAPI`) chamados no topo das renders (agora `async`). `_mergeRFQs`/`_getMatrizes` **injetam o cache da API** mesclado com as linhas locais legadas (dedup por número, nada some); a escrita (`_saveMatrizes` e o writeback de `_mergeRFQs`) **nunca persiste linhas da API** (anti-duplicação). **Ações reais**: aprovar/reprovar mapa da API chamam `POST /api/mapas/:id/aprovar|reprovar` (o servidor aplica perfil/alçada/concorrência), com fallback ao fluxo local quando offline. Degrada em silêncio sem servidor (modo demo por máquina). Emissão de RFQ/PC segue como próxima fase | ✅ front + testes |
| Testes | ✅ 1097/1097 (segurança, gate, bridge, crédito, sync genérico, NexusAPI, compras E2E, nav_safety, multi-tenant Express+Worker, hardening, dashboard dados reais, badge tenant real, health/dashboard, gestão de tenants, anomalias em alertas, DRE real, conciliação bancária CSV/OFX, RH/mão de obra na DRE, margem real por contrato, fluxo de caixa projetado, dashboard financeiro consolidado, exportação CSV/PDF, orçamento anual budget×realizado, CSV formula injection, estoque custo médio+reposição, reposição→RC, RC→pedido de compra, **portal RFQ self-service**, SSMA TF/TG + EPIs/NR-6 + matriz NR/aptidão + CAT/S-2210, **MM completo + varredura + MRP→RC + painel CEO**, **PP completo: ordens + calendário/custo→margem + 2 varreduras**, **WMS endereçamento + picking**, **auditoria do menu + export .xlsx Processos**, **seed demo abrangente + top-up idempotente**, **RFQ/Mapa→API**, paridade Express⇄Worker) |
