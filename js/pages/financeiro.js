...
async function loadContasPagar() {
  try {
    const res = await fetch('tables/fa_contas_pagar?limit=200');
    const data = await res.json();
    FA_CONTAS_PAGAR = data.data || [];
    window.FA_CONTAS_PAGAR = FA_CONTAS_PAGAR;
  } catch(e) { FA_CONTAS_PAGAR = []; }

  // Sem fallback de dados fictícios – lista vazia quando API retorna nada
}
...
async function salvarEdicaoCP(id) {
  ...
  try { await fetch(`tables/fa_contas_pagar/${id}`, { method: 'PATCH', headers: {'Content-Type':'application/json'}, body: JSON.stringify(FA_CONTAS_PAGAR[idx]) }); } catch(e) {}
  closeModal();
...
async function salvarNovaCP() {
  ...
  try {
    await fetch('tables/fa_contas_pagar', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify(nova) });
  } catch(e) {}
