...
function renderBuscaGlobal() {
  ...
  el.innerHTML = `
    ...
    <div style="background:var(--bg-card);border-radius:16px;border:2px solid var(--border-color);padding:24px;margin-bottom:16px;box-shadow:0 4px 24px rgba(0,0,0,0.08)">
      <div style="display:flex;gap:12px;align-items:center;margin-bottom:16px">
        <div style="position:relative;flex:1">
          ...
          <input type="text" id="bg_search_input"
            placeholder="Digite OS-001, CNPJ, nome do fornecedor, contrato..."
            value="${_bgQuery}"
            style="width:100%;padding:14px 16px 14px 48px;font-size:16px;border:2px solid var(--border-color);border-radius:12px;background:var(--bg-tertiary);color:var(--text-primary);outline:none;transition:all .2s;box-sizing:border-box"
            oninput="_bgOnInput(this.value)"
            onkeydown="if(event.key==='Enter'){_bgBuscar(this.value)}"
            onfocus="this.style.borderColor='var(--primary)';this.style.boxShadow='0 0 0 4px rgba(79,70,229,0.12)'"
            onblur="this.style.borderColor='var(--border-color)';this.style.boxShadow='none'"
            autocomplete="off">
          ${_bgQuery ? `<button onclick="_bgLimpar()" style="position:absolute;right:14px;top:50%;transform:translateY(-50%);border:none;background:var(--bg-tertiary);color:var(--text-muted);cursor:pointer;font-size:18px;width:28px;height:28px;border-radius:50%;display:flex;align-items:center;justify-content:center;line-height:1" title="Limpar">×</button>` : ''}
        </div>
        <button onclick="_bgBuscar(document.getElementById('bg_search_input').value)" class="btn btn-primary" style="padding:14px 28px;font-size:15px;border-radius:12px;white-space:nowrap;font-weight:700">
      ...
      <div style="display:flex;gap:6px;flex-wrap:wrap;align-items:center;margin-bottom:10px">
        ...
        ${tipos.map(f => `
          <button onclick="_bgSetFiltro('tipo','${f.k}')" 
            style="padding:5px 14px;border-radius:20px;border:1.5px solid ${_bgFiltros.tipo===f.k ? 'var(--primary)' : 'var(--border-color)'};background:${_bgFiltros.tipo===f.k ? 'var(--primary)' : 'transparent'};color:${_bgFiltros.tipo===f.k ? '#fff' : 'var(--text-secondary)'};font-size:12px;font-weight:600;cursor:pointer;transition:all .18s;white-space:nowrap">
      ...
      <div style="display:flex;gap:6px;flex-wrap:wrap;align-items:center">
        ...
        ${periodos.map(p => `
          <button onclick="_bgSetFiltro('periodo','${p.k}')"
            style="padding:5px 14px;border-radius:20px;border:1.5px solid ${_bgFiltros.periodo===p.k ? 'var(--primary)' : 'var(--border-color)'};background:${_bgFiltros.periodo===p.k ? 'var(--primary)' : 'transparent'};color:${_bgFiltros.periodo===p.k ? '#fff' : 'var(--text-secondary)'};font-size:12px;font-weight:600;cursor:pointer;transition:all .18s">
            ${p.l}
          </button>
        `).join('')}
      </div>
