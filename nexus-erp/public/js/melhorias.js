...
// ─── 12. BUSCA RÁPIDA NO TOPBAR — sempre visível, live dropdown ──────────────

// Compatibilidade retroativa (funções chamadas pelo HTML antigo)
...
function _closeTopbarSearch() {
  const inp = document.getElementById('topbar_quick_search');
  const dd  = document.getElementById('topbar_search_dropdown');
  const clr = document.getElementById('topbar_search_clear');
  if (inp) inp.value = '';
  if (clr) clr.style.display = 'none';
  if (dd)  dd.style.display  = 'none';
}

// Busca live: alimenta o dropdown enquanto digita
function _topbarSearchLive(val) {
  const clr = document.getElementById('topbar_search_clear');
  const dd  = document.getElementById('topbar_search_dropdown');
  if (clr) clr.style.display = val ? 'block' : 'none';
  if (!dd) return;
  if (!val || val.trim().length < 2) { dd.style.display = 'none'; return; }
  dd.style.display = 'block';
  dd.innerHTML = _topbarBuscarRapido(val.trim());
}

// Motor de busca rápida para o dropdown
function _topbarBuscarRapido(q) {
  ...
  const relevancia = (t) => {
    ...
    if (s === ql) return 20;
    if (s.startsWith(ql)) return 10;
    if (s.includes(ql)) return 5;
    return 0;
  };

  // Coleta de fontes (reusa coletores do busca_global se disponíveis)
  const fontes = [];
  try { if (typeof _bgColetarOS === 'function')        [..._bgColetarOS().values()].forEach(o => fontes.push({ tipo:'os', icone:'fa-clipboard-list', cor:'#2563eb', label:'OS', id:o.id, titulo:o.descricao||o.id, sub:[o.tipo,o.contrato,o.status].filter(Boolean).join(' · '), nav:'os', score: relevancia([o.id,o.numero,o.descricao,o.contrato,o.responsavel].join(' ')) })); } catch(e){}
  try { if (typeof _bgColetarRC === 'function')        [..._bgColetarRC().values()].forEach(r => fontes.push({ tipo:'rc', icone:'fa-file-alt', cor:'#7c3aed', label:'RC', id:r.id, titulo:r.titulo||r.descricao||r.objeto||r.id, sub:[r.solicitante,r.contrato,r.status].filter(Boolean).join(' · '), nav:'requisicoes', score: relevancia([r.numero,r.titulo,r.descricao,r.objeto,r.solicitante,r.contrato].join(' ')) })); } catch(e){}
  try { if (typeof _bgColetarPedidos === 'function')   [..._bgColetarPedidos().values()].forEach(p => fontes.push({ tipo:'pedido', icone:'fa-shopping-bag', cor:'#16a34a', label:'Pedido', id:p.id, titulo:(p.fornecedor_nome||p.fornecedor||'Pedido'), sub:[p.contrato_id||p.contrato,p.status].filter(Boolean).join(' · '), nav:'pedidos', score: relevancia([p.numero,p.id,p.fornecedor_nome,p.fornecedor,p.contrato_id].join(' ')) })); } catch(e){}

  // Fornecedores (fontes múltiplas)
  try {
    const fm = new Map();
    ['fa_fornecedores','fa_fornecedores_cache'].forEach(k => { try { JSON.parse(localStorage.getItem(k)||'[]').forEach(f => { if(f.id&&!fm.has(f.id)) fm.set(f.id,f); }); } catch(e){} });
    if (typeof FA_FORNECEDORES !== 'undefined') FA_FORNECEDORES.forEach(f => { if(f.id&&!fm.has(f.id)) fm.set(f.id,f); });
    [...fm.values()].forEach(f => fontes.push({ tipo:'fornecedor', icone:'fa-building', cor:'#d97706', label:'Fornecedor', id:f.id, titulo:f.razao_social||f.nome_fantasia||f.id, sub:[f.categoria,f.cidade].filter(Boolean).join(' · '), nav:'fornecedores', score: relevancia([f.razao_social,f.nome_fantasia,f.cnpj,f.cidade,f.categoria].join(' ')) }));
  } catch(e){}

  // Contratos (ERP_DATA)
  try {
    if (typeof ERP_DATA !== 'undefined' && Array.isArray(ERP_DATA.contratos)) {
      ERP_DATA.contratos.forEach(c => fontes.push({ tipo:'contrato', icone:'fa-file-contract', cor:'#dc2626', label:'Contrato', id:c.id, titulo:c.objeto||c.cliente||c.id, sub:[c.cliente,c.status].filter(Boolean).join(' · '), nav:'contratos', score: relevancia([c.id,c.objeto,c.cliente,c.numero].join(' ')) }));
    }
  } catch(e){}

  const filtrados = fontes.filter(f => f.score > 0).sort((a,b) => b.score - a.score).slice(0, 8);

  if (!filtrados.length) {
    return `<div style="padding:20px;text-align:center;color:var(--text-muted);font-size:13px">
      <i class="fas fa-search" style="font-size:22px;margin-bottom:8px;display:block;opacity:.4"></i>
      Nenhum resultado para "<strong style="color:var(--text-primary)">${_escHtml(q)}</strong>"
      <br><button onclick="document.getElementById('topbar_search_dropdown').style.display='none';_bgQuery='${_escHtml(q)}';navigate('busca_global')" style="margin-top:10px;padding:6px 14px;border-radius:8px;border:1px solid var(--border);background:var(--bg-dark);color:var(--text-secondary);cursor:pointer;font-size:12px">
        <i class="fas fa-search" style="margin-right:4px"></i>Busca completa
      </button>
    </div>`;
  }

  const items = filtrados.map(r => `
    <div onclick="document.getElementById('topbar_search_dropdown').style.display='none';document.getElementById('topbar_quick_search').value='';navigate('${r.nav}')"
      style="display:flex;align-items:center;gap:10px;padding:10px 14px;cursor:pointer;transition:background .15s;border-radius:0"
      onmouseover="this.style.background='var(--bg-dark)'" onmouseout="this.style.background='transparent'">
      <div style="width:32px;height:32px;border-radius:8px;background:${r.cor}18;display:flex;align-items:center;justify-content:center;flex-shrink:0">
        <i class="fas ${r.icone}" style="color:${r.cor};font-size:13px"></i>
      </div>
      <div style="flex:1;min-width:0">
        <div style="font-size:12px;font-weight:700;color:${r.cor};text-transform:uppercase;letter-spacing:.4px">${r.label} · <span style="color:var(--text-muted);font-weight:500">${_escHtml(r.id)}</span></div>
        <div style="font-size:13px;font-weight:600;color:var(--text-primary);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${_escHtml(r.titulo)}</div>
        ${r.sub ? `<div style="font-size:11px;color:var(--text-muted);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${_escHtml(r.sub)}</div>` : ''}
      </div>
      <i class="fas fa-arrow-right" style="color:var(--text-muted);font-size:10px;flex-shrink:0"></i>
    </div>`).join('');

  const footer = `
    <div style="border-top:1px solid var(--border);padding:8px 14px">
      <button onclick="document.getElementById('topbar_search_dropdown').style.display='none';_bgQuery='${_escHtml(q)}';navigate('busca_global')"
        style="width:100%;padding:7px;border-radius:8px;border:1px solid var(--border);background:var(--bg-dark);color:var(--text-secondary);cursor:pointer;font-size:12px;font-weight:600;transition:background .15s"
        onmouseover="this.style.background='var(--bg-tertiary)'" onmouseout="this.style.background='var(--bg-dark)'">
        <i class="fas fa-search" style="margin-right:6px;color:var(--orange)"></i>Ver todos os resultados para "<strong>${_escHtml(q)}</strong>"
      </button>
    </div>`;

  return `<div style="padding:8px 0 0">${items}</div>${footer}`;
}
...
document.addEventListener('keydown', function(e) {
  ...
  if (e.key === 'Escape') { _closeTopbarSearch(); }
});
